...
if (arguments.length > 0) {
	...
	fs.readFile("profile.lombardia.js", "UTF-8", function(err, data) {
		if (err)
			throw err;
		start(JSON.parse(data));
	});
...
function start(jConfig) {
	...
	http.get(jConfig.packageListUrl, function(res) {
		...
		res.on('end', function() {
			...
			for ( var int = 0; int < resArray.length; int++) {
				if (resArray[int][15][0]) { // se esistono le api allora è un
					// dataset
					singleDataset(resArray[int][13], jConfig);
				}
				// return;
			}
		});
	...
	function singleDataset(dataId, jConfig) {
		console.log("\n\nprocessing " + jConfig.singlePackageBaseUrl + dataId);

		http.get(jConfig.singlePackageBaseUrl + dataId + ".json", function(res) {
			res.setEncoding('utf8');
			var resText = "";
			res.on('data', function(chunk) {
				resText += chunk;
		...
		if (params.type == 'map') {
			...
			for ( var int = 0; int < resJ[key].length; int++) {
				if (resJ[key][int]) {
					...
					row = staticInfo(resourceUri, val, params) + row;
					val = val.replace(/"/gi, '\\"');
					if(val==''){
						continue;
					}
					if (params.type == 'string') {
						...
						row += params.uri + '\t<' + generateUri((params.prefix ? params.prefix : "") + val, params) + '>;\t';
					} else {
						row += params.uri + '\t' + val + ';\t';
					}
				}
